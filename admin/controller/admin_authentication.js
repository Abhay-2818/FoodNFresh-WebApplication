       ...
       // console.log(email,"   ",password);
        ...
        Admin.findOne( {email} , async (err,admin) => {
            ...
            if(err || !admin)
            {
                ...
                req.flash('admin_login_errormsg', "User with this email is doesn't exist . please sign-in");
                res.redirect("/admin/admin_login");
            }
            else{
                ...
                if(x)
                {
                    const token = jwt.sign(
                        {_id: admin._id}, process.env.JWT_SECRET );
                    ...
                    req.flash('admin_login_errormsg', "email and password are not matching");
                    res.redirect("/admin/admin_login");
    ...
    else
    {
        ...
        if(password === cpassword){
            ...
            if(adminexists){
                ...
                req.flash('admin_register_errormsg', "Email is already taken signup with different email");
                res.redirect("/admin/admin_register");
            ...
            else
            {
                ...
                await admin.save();
                req.flash('admin_register_errormsg', "Account created successfully. Please Login.");
                res.redirect("/admin/admin_register");
            }
