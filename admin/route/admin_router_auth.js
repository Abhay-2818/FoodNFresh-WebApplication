...
const {admin_loginpage, admin_registerpage , admin_register , admin_login} = require("../controller/admin_authentication");
const {admin_logout,admin_profile,admin_changepw,admin_profile_page} = require('../controller/admin');
...
router.post('/admin_register',[
    ...
    check("password").isLength({min:6}).withMessage("Password must contain at least 6 characters").matches(/\d/)
...
const storage = multer.diskStorage({
    destination: (req,file,callback) => {
        callback(null, "./admin/public/admin_src/assets/images/admin_profile_images");
    },
...
router.post("/admin_profile_upload",upload.single("originalname"),admin_profile_photo);
