...
const {admin_grocery_details,admin_grocery_details_page} = require('../controller/admin_grocery_details')
const {admin_customer_list_page}  = require('../controller/admin_customer_list');
const {admin_pending_order_list_page , admin_processing_order_list_page,
admin_shipped_order_list_page,admin_completed_order_list_page,admin_reject_order_list_page,
admin_order_details_page} = require('../controller/admin_order_list');

const {admin_add_product_category_page,add_product_category,admin_all_category_page,admin_category_status_change} = require('../controller/admin_category');
const {admin_add_new_product_page,admin_add_product,admin_all_product_page,admin_all_product_status_change} = require('../controller/admin_product');
...
router.get("/admin_all_category/:category_id/:status",admin_category_status_change);
...
const upload1 = multer({
    storage : storage1,fileFilter(req,file, cb) {
        if (!file.originalname.match(/\.(png|jpg|jpeg)$/)) { 
           // upload only png and jpg format
           return cb(new Error('Please upload a jpg | png | jpeg Image file ')) 
        }
...
router.get("/admin_all_product/:product_id/:status",admin_all_product_status_change);
