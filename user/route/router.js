...
const {loginpage , registerpage , signin , signup, forgot_password_page,forgot_password} = require('../controller/authentication');
const {userdashboard, account_setting, logout , updateuserinfo, change_pw} = require('../controller/user');
const {category} = require('../controller/category');
const {order_details_page , order_history_page ,order} = require('../controller/order');
...
router.post("/profile_upload",upload.single("originalname"),(req,res) => {
    ...
    if(JSON.stringify(req.file) == undefined)
    {
      ...
      user.findOneAndUpdate({ _id: userId },
        ...
        ,(err, doc) => {
        if (!err) { res.redirect("/user_dashboard"); }
        else {
