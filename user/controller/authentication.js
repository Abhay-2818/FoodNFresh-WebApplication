    ...
    if(!errors.isEmpty()){
       ...
       // console.log(email,"   ",password);
        ...
        User.findOne( {email} , async (err,user) => {
            ...
            if(err || !user)
            {
                req.flash('login_errormsg', "User with this email is doesn't exist . please sign-in");
                res.redirect("/login");
            }
            else{
    ...
    else
    {
        ...
        if(password === cpassword){
            ...
            if(userexists){
                ...
                req.flash('register_errormsg', "Email is already taken signup with different email");
                res.redirect("/register");
            ...
            else
            {
                ...
                //console.log(user);
                await user.save();
                req.flash('register_errormsg', "Account created successfully. Please Login.");
                res.redirect("/register");
    ...
    if(!errors.isEmpty()){
        ...
        if(password === cpassword)
        {
             ...
             User.findOneAndUpdate({ email: email },
                ...
                ,(err, doc) => {
                if (!err) { 
                    req.flash('successful',"Successfully changed password. Please Login.")
                    res.redirect("/forgot_password"); 
                }
                else {
